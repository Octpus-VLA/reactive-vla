...
- `scene_cameras.xml` … SmolVLA 用カメラ（`cam1`/`cam2`）と操作対象の赤キューブ（2.5cm, free joint, `pos="0.2 0 0.0125"`）を足したもの。**ロールアウトではこれを使う**
- `scene.xml` … upstream そのまま（カメラ・操作対象なし）
...
> カメラ位置・向き、およびキューブの初期位置は `scene_cameras.xml` 内の暫定値。実機の撮影アングルやアームの作業範囲に合わせて要調整（`mujoco.viewer` で当たりを付けると早い）。
