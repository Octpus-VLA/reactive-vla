...
リアクティブ制御は3段階の再計画（reactivity）で構成されています。

- **Tier 1**: 既存のアクションキュー残量に基づく replan。
- **Tier 2**: カメラ supervisor によるイベント駆動の早期 replan。
- **Tier 3**: cube の位置・速度・推定把持タイミングから動的に有効ホライズンを決める予測的 replan。
