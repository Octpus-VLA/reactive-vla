# SO-101 CLI Reference

English | [日本語](README.md)

`cli/so101.py` is the SO-101 hardware/simulation CLI, exposed as `pixi run <command>`. This page is the detailed command and flag reference. For project overview and environment setup, see the [root README](../README-en.md).

## Registering the SO-101 arms
...
pixi run train \
  --policy-path lerobot/smolvla_base --repo-id OctpusVLA/<dataset> \
...
For moving-cube trials, the RTC overhead predictor can advance the red cube forward by the inference latency (PE gap) on the overhead camera and feed the time-advanced frame to the policy (see [docs/overhead-predictor.md](../docs/overhead-predictor.md)):
