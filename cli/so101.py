...
def sim_eval(
    ...
    Tier 1's queue-only trigger would. --supervisor-disable-predictor (on by
    default) also mutes --predict-cube's forward-shift for that tick, since once
    the cube is imminently within reach the real current frame matters more than
    a lead-ahead extrapolation; --supervisor-execution-horizon can additionally
    shrink the chunk commit length at that point.
    """
